...
impl Cha {
	...
	pub fn permissions(&self) -> String {
		let kind = if self.is_dir { 'd' } else if self.is_link { 'l' } else { '-' };
		let bit = |mask: u32, ch: char| if self.mode & mask != 0 { ch } else { '-' };

		let mut out = String::with_capacity(10);
		out.push(kind);
		for (r, w, x, special, special_ch) in [
...
mod tests {
	...
	fn formats_a_directory_with_setgid_and_others_writable() {
		let cha = Cha { len: 0, is_dir: true, is_link: false, modified: None, mode: 0o2775 };
		assert_eq!(cha.permissions(), "drwxrwsr-x");
	}

	#[test]
	fn formats_a_plain_file_with_no_group_or_other_access() {
		let cha = Cha { len: 0, is_dir: false, is_link: false, modified: None, mode: 0o600 };
