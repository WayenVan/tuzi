...
impl CompletionPopup {
	pub fn render(frame: &mut Frame, area: Rect, anchor: Rect, candidates: &[String], selected: usize) {
		if candidates.is_empty() {
			return;
		}

		let height = (candidates.len().min(8) as u16 + 2).min(area.height);
		...
		let items = candidates.iter().map(|name| ListItem::new(format!("{name}{}", std::path::MAIN_SEPARATOR)));
